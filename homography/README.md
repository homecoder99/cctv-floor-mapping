...
- 버전 관리
- **NEW (v3.0)**: DXF CAD 좌표계 기반 호모그래피 (렌더 크기 독립적)
...
**최신 버전 (v3.0 - CAD 좌표 기반):**
```bash
python point_collector_v2.py
```

**레거시 버전 (v1.0 - 픽셀 좌표 기반):**
```bash
python point_collector.py
```

> **권장**: `point_collector_v2.py` 사용 (CAD 좌표 기반으로 렌더 크기 변경 시에도 일관성 유지)
...
### Version 3.0 (CAD 좌표 기반) - 권장

```json
{
  "metadata": {
    "created_at": "2025-12-24T17:57:11",
    "updated_at": "2025-12-24T17:57:11",
    "version": "3.0",
    "image_source": "/path/to/frame.jpg",
    "drawing_source": "/path/to/house.dxf",
    "calibration_source": "/path/to/calibration_data.pkl",
    "undistort_applied": true,
    "num_points": 8,
    "total_points": 8,
    "active_points": 8,
    "dxf_bounds": [15000.0, 20000.0, 45000.0, 50000.0],
    "dxf_y_inverted": true,
    "dxf_output_size": [3000, 3000],
...
- `dxf_bounds`: DXF 도면의 실제 좌표 범위 (min_x, min_y, max_x, max_y)
- `dxf_y_inverted`: Y축 반전 여부 (렌더링 시 -y 적용)
- `dxf_output_size`: 렌더링된 이미지 크기
- `coordinate_system`: "CAD" (좌표계 명시)

### Version 1.0 (픽셀 좌표 기반) - 레거시
...
```json
{
  ...
  "image_points": [[100.5, 200.3], ...],
  "drawing_points": [[1500.0, 2000.0], ...],  // 픽셀 좌표 (0~3000 범위)
  "homography_matrix": [...],
  "reprojection_error": {...}
}
```
...
**문제점 (v1.0)**:
- `drawing_points`가 픽셀 좌표로 저장됨
- 렌더 크기 변경 시 좌표계 불일치 발생
- 실시간 추적 시 좌표 매핑 오류 가능
...
### Version 3.0 (2025-12-24) - CAD 좌표 기반

**변경사항**:
1. **DXF 렌더러 개선** (`dxf_renderer.py`)
   - `return_metadata=True` 옵션 추가
   - DXF 원본 좌표 범위 (bounds) 반환
   - Y축 반전 정책 (`y_inverted`) 반환
   - 렌더링 크기 정보 제공

2. **GUI 픽셀→CAD 좌표 변환** (`point_collector_v2.py`)
   - `pixel_to_cad()`: 렌더링 픽셀 → DXF CAD 좌표
   - `cad_to_pixel()`: DXF CAD 좌표 → 렌더링 픽셀
   - 클릭 시 자동으로 CAD 좌표로 저장
   - 점 삭제/Undo/Redo 시 자동 변환

3. **호모그래피 매핑**
   - **이전**: 영상 픽셀 → 도면 픽셀 (0~3000)
   - **현재**: 영상 픽셀 → DXF CAD 좌표 (mm 단위)

4. **장점**
   - 렌더 크기 독립적 (3000→1500 변경 시에도 동작)
   - Y축 정책 명확 (메타데이터에 기록)
   - 좌표계 일관성 (CAD 기준)
   - Zone polygon 정합 개선

**호환성**:
- v1.0 JSON → v3.0: 직접 호환 불가 (좌표계 다름)
- v3.0 JSON → 실시간 추적: 완벽 호환
- 기존 호모그래피는 재생성 권장

### Version 1.0 (2024-12-24) - 픽셀 좌표 기반

**특징**:
- 도면 점을 렌더링 이미지의 픽셀 좌표로 저장
- 간단하지만 렌더 크기에 종속적
- 좌표계 불일치 가능

**문제점**:
- GUI 렌더 크기 (3000x3000) ≠ 실시간 렌더 크기 (1500x1500) → 2배 스케일 차이
- `drawing_pos`가 픽셀 범위인데 `drawing_bounds`는 CAD 범위 → 좌표계 불일치
- 1D 이동 현상 발생 가능

## 마이그레이션 가이드 (v1.0 → v3.0)

기존 v1.0 호모그래피를 사용 중이라면:

**옵션 A: 새로 생성 (권장)**
```bash
python point_collector_v2.py
# 이미지, DXF 선택 후 대응점 다시 선택
# 자동으로 CAD 좌표로 저장됨
```

**옵션 B: 임시 해결 (레거시)**
```bash
# 실시간 추적 시 GUI와 동일한 렌더 크기 사용
python realtime_tracking/tracker.py \
  --homography homography/data/homography_old.json \
  --drawing-size 3000 3000  # GUI와 동일
  --flip-drawing-y
```
...
**Version 3.0 사용 시**:
- `transform_to_drawing()` → DXF CAD 좌표 반환
- `drawing_coord_to_pixel()` → 화면 표시용 픽셀 변환
- Zone polygon을 CAD 좌표로 정의
